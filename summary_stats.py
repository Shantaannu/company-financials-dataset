...
# 6. Grouped Insights
segment_perf = financial_dataset_clean.groupby("Segment")[["Profit (in $)", "Sales (in $)"]].sum().sort_values(by="Profit (in $)", ascending=False)
print("\nSegment Performance:\n", segment_perf)

country_perf = financial_dataset_clean.groupby("Country")[["Profit (in $)", "Sales (in $)"]].sum().sort_values(by="Profit (in $)", ascending=False)
print("\nCountries by Profit:\n", country_perf.head(10))

product_perf = financial_dataset_clean.groupby("Product")[["Profit (in $)", "Sales (in $)"]].sum().sort_values(by="Profit (in $)", ascending=False)
print("\nProducts by Profit:\n", product_perf)
